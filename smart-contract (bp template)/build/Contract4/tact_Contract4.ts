...
function dictValueParserWithdraw(): DictionaryValue<Withdraw> {
 ...
 type Contract4_init_args = {
    $$type: 'Contract4_init_args';
}

function initContract4_init_args(_src: Contract4_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
    };
}

async function Contract4_init() {
...
const Contract4_errors: { [key: number]: { message: string } } = {
...
export class Contract4 implements Contract {
    
    static async init() {
        return await Contract4_init();
    }
    
    static async fromInit() {
        const init = await Contract4_init();
        const address = contractAddress(0, init);
        return new Contract4(address, init);
    }
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: null | 'increment' | 'refund increment' | Withdraw | Deploy) {
